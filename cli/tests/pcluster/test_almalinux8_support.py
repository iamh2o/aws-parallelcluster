...
IMAGEBUILDER_RESOURCES = Path(__file__).resolve().parents[2] / "src" / "pcluster" / "resources" / "imagebuilder"
...
def test_almalinux8_configuration_contract():
    assert_that(SUPPORTED_OSES).contains("alinux2")
    assert_that(SUPPORTED_OSES).contains("almalinux8")
    assert_that(SUPPORTED_OSES_FOR_SCHEDULER["awsbatch"]).is_equal_to(["alinux2", "alinux2023"])
